...
def build_live_network_figure(routes_json, hubs_json):
    ...
    fig.update_layout(template="plotly_dark", height=520, margin=dict(l=0,r=0,t=0,b=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, uirevision="live_map_cached")
    return fig
...
with c1:
    ...
    try:
        ...
        st.warning("Live tactical route network could not render on this PC. Showing safe route table instead.")
        st.caption(f"Render detail: {type(e).__name__}: {e}")
        fallback_cols = [c for c in ["Origin", "Destination", "Risk", "Volume", "Status"] if c in routes.columns]
        st.dataframe(routes[fallback_cols].head(20), use_container_width=True, hide_index=True)
